...
A React dev build for exercising devharness annotate mode. `npm install && npm run dev` serves it on 3102.
...
annotate mode exists to hold open.
...
correct. annotate mode checks the reported position against the file and searches by column and tag when
